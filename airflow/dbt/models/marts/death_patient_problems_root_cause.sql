...
with source as (
    select * from {{ ref('death_patient_problems_category') }}
),

death_patient_problems_root_cause as (
    select
        event_key,
        problem_category,
...
select event_key, problem_category from death_patient_problems_root_cause